...
**Дополнение (на дополнительную оценку):** заменить кнопку S1 на потенциометр с аналоговым сигналом
...
## Задание 17 (FB блок на LAD)

Создать Function Block на языке LAD, главный блок (Main) должен быть на языке LAD.

Оформление блока: 2 входа и 1 выход

Условие блока: внутри блока сделать схему мигание лампочки с частотой 1Hz. Если на первый вход блока подать короткий сигнал, то лампочка начинает мигать. Лампочка соединяется с выходом блока FB. Если на второй вход подать короткий сигнал, то лампочка перестает мигать.&#x20;
...
**Условие:** при коротком нажатии на кнопку S1 лампочка Q1 начинает мигать с частотой 1Hz, при повторном нажатии на S1 лампочка Q1 перестает мигать. Программу сделать на SCL

## Задание 19 (Свой блок на SCL)
...
## Задание 20 (Количество миганий лампочки на SCL)

У вас имеются кнопки S1, S2, S3, так же имеется 8 лампочек (L1 ... L8). Первая (S1) кнопка переключается между лампочками (активная лампочка светится), после выбора последней лампочки круг повторяется. Вторая (S2) кнопка включает режим мигания у текущей выбранной лампочки, короткое нажатие кнопки S3 выключает режим мигания у текущей лампочки.

**Сделать на языке SCL**

## Задание 21 (Выбор и мигание лампочки SCL)
...
## Задание 22 (Задание по листочку)

Спросить у учителя

## Задание 23 (Светофор SCL)
...
Обозначения: S1, S2 - кнопки с коротким нажатием. Q1, Q2 Q3 - сигналы светофора для машины, Q4 и Q5 - сигналы светофора для пешехода
...
## Задание 24 (Панель управления)

Имеются кнопки: S1 (Start), S2 (Stop), S3 (Reset), S4 (On/Off)

Имеются лампочки: Q1 (Start LED), Q2 (Reset LED)

Кнопки: S1, S2, S3 - импульсные, кнопка S4 с фиксацией.

**Условие:** чтобы вся система работала, необходимо перевести S4 в верхнее положение (в режим ON). Когда система включена, то Q1 начинает мигать с частотой 1Hz, это значит, что система готова к запуску. Когда происходит нажатие на кнопку S1, то Q1 перестает мигать и начинает просто гореть, что значит, что система работает. Если во время работы системы нажать на S2, то Q1 перестает гореть и начинает гореть Q2, это значит, что система заблокирована. Чтобы разблокировать систему, необходимо нажать на S3 и тогда Q2 перестает гореть, а Q1 начинает мигать и цикл повторяется.&#x20;

Если во время режима "готов к запуску" и "система работает" переключить S4, то Q1 и Q2 выключаются. Когда снова переключаем S4 то система начинает работать и цикл повторяется с миганием Q1.
