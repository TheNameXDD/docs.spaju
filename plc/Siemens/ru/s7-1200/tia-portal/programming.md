...
#### TON таймер (или таймер на включение)

<figure><img src="../../../../../.gitbook/assets/TIA_ton.png" alt=""><figcaption></figcaption></figure>

**TON таймер** при получении сигнала на **IN** начинает отсчитывать определенное количество времени, заданное в **PT**. В **ET** таймер выводит актуальное значение времени, которое прошло с момента получения сигнала на **IN**. Когда время указанное в **PT** было отсчитано таймер включает выход **Q**, который в свою очередь передает сигнал (1 или 0) на следующие блоки.&#x20;

{% hint style="info" %}
Если во время отсчета таймера пропадет сигнал на входе **IN**, то таймер автоматически сбрасывается.&#x20;
{% endhint %}

Пример реализации на SCL:

```cpp
"Timer_TON".TON(IN := "BUTTON", //IN ждет появление сигнала от BUTTON
                PT := T#3s, //Указываем время, которое должен отсчитать таймер
                Q => "LED"); //Куда таймер передает сигнал
```

#### TOF таймер (или таймер на выключение)
